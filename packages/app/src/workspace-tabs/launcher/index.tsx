...
export function useWorkspaceTabLaunchCatalog(input: {
  ...
  const presentations = useMemo(
    () => resolveBuiltInLaunchPresentations(surfaceConfig, getLaunchPresentation),
    [surfaceConfig],
  );
  const showChanges = launcher.showChanges && surfaceConfig.showGitChanges;
  const showDiff = launcher.showChanges && surfaceConfig.showDiff;
  const showBrowser = launcher.showBrowser && surfaceConfig.showBrowser;
  const visiblePlugins = surfaceConfig.showPluginClientUi ? plugins : EMPTY_PLUGINS;
  ...
  return useMemo(() => {
    const {
      changes: changesPresentation,
      diff: diffPresentation,
      files: filesPresentation,
    ...
    const builtIns: Record<BuiltInLaunchItemId, WorkspaceTabLaunchItem & { hidden?: boolean }> = {
      ...
      files: {
        ...
        hidden: !surfaceConfig.showFileExplorer,
