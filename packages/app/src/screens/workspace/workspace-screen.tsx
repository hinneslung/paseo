...
import {
  ...
  isWorkspaceTabTargetSupported,
  type WorkspaceSurfaceConfig,
} from "@/workspace/surface-capabilities";
...
function hasExplorerSurface(config: WorkspaceSurfaceConfig): boolean {
  return config.showFileExplorer || config.showGitChanges;
}

function WorkspaceExplorerSurface({
  visible,
  children,
}: {
  visible: boolean;
...
function resolveWorkspaceLauncherVisibility(
  config: WorkspaceSurfaceConfig,
  isGitCheckout: boolean,
): { showBrowser: boolean; showChanges: boolean } {
  return {
    showBrowser: getIsElectron() && config.showBrowser,
    showChanges: config.showGitChanges && isGitCheckout,
  };
}
...
function WorkspaceScreenContent({
  ...
  const surfaceConfig = useMemo(getWorkspaceSurfaceConfig, []);
  const showExplorerSurface = hasExplorerSurface(surfaceConfig);
  ...
  const handleWorkspacePanelOpenAction = useCallback(
    (action: KeyboardActionDefinition): boolean => {
      ...
      if (!isWorkspaceTabTargetSupported(target, surfaceConfig)) return true;
    ...
    (action: KeyboardActionDefinition): boolean => {
      switch (action.id) {
        ...
        case "workspace.browser.new":
          if (surfaceConfig.showBrowser) handleCreateBrowserTab();
          return true;
  ...
  const handleWorkspaceDirectTargetAction = useCallback(
    (action: KeyboardActionDefinition): boolean => {
      ...
      switch (action.id) {
        ...
        case "workspace.tab.target.browser":
          if (surfaceConfig.showBrowser) handleCreateBrowserTab({ paneId });
          return true;
        case "workspace.tab.target.changes":
          if (surfaceConfig.showGitChanges && persistenceKey && isGitCheckout) {
            openExplorerSidebarView({
              isCompact: isMobile,
              workspaceKey: persistenceKey,
              checkout: activeExplorerCheckout,
        ...
        case "workspace.tab.target.files":
          if (surfaceConfig.showFileExplorer && persistenceKey) {
            openExplorerSidebarView({
              isCompact: isMobile,
              workspaceKey: persistenceKey,
              checkout: activeExplorerCheckout,
    ...
    [
      ...
      persistenceKey,
      surfaceConfig.showBrowser,
      surfaceConfig.showFileExplorer,
      surfaceConfig.showGitChanges,
    ],
  );
  ...
  const handleWorkspaceSidebarAction = useCallback(
    (action: KeyboardActionDefinition): boolean => {
      ...
      if (!showExplorerSurface) return true;
    ...
    [handleToggleExplorerSidebar, isMobile, persistenceKey, showExplorerSurface],
  );
      ...
      <View style={styles.headerRight}>
        ...
        {!isMobile && workspaceDirectory ? (
          <>
            ...
            <WorkspaceExplorerSurface visible={showExplorerSurface}>
              <WorkspaceHeaderExplorerToggle
              ...
              />
            </WorkspaceExplorerSurface>
          </>
        ) : null}
        <WorkspaceExplorerSurface visible={isMobile && showExplorerSurface}>
          <WorkspaceExplorerToggle
            onPress={handleToggleExplorerSidebar}
            label={explorerSidebarToggleLabel}
            tooltipLabel={t("workspace.tabs.explorerSidebar.toggle")}
  ...
  const launcherVisibility = resolveWorkspaceLauncherVisibility(surfaceConfig, isGitCheckout);
  const showCreateBrowserTab = launcherVisibility.showBrowser;
  const newTabLauncher = useMemo<NewTabLauncher>(
    () => ({
      showChanges: launcherVisibility.showChanges,
    ...
    [
      createTerminalDisabled,
      hasPullRequest,
      launcherVisibility.showChanges,
      launchWorkspaceTab,
      showCreateBrowserTab,
    ],
  );
