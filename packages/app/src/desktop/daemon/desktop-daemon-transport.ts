...
export function buildDesktopDaemonTransportUrl(target: DesktopDaemonTransportTarget): string {
  ...
  } else if (target.transportType === "ssh") {
...
export function parseDesktopDaemonTransportUrl(url: string): DesktopDaemonTransportTarget {
...
export function createDesktopDaemonTransportFactory(
  ...
  return ({ url, protocols }) => {
    const parsedTarget = parseDesktopDaemonTransportUrl(url);
    const target =
      parsedTarget.transportType === "tcp" && protocols && protocols.length > 0
        ? { ...parsedTarget, protocols }
        : parsedTarget;
